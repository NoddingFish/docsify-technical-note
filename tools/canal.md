...
6. #### 数据库中 `datetime` 默认为 `0000-00-00 00:00:00` 的问题
   ...
   1. ?> 直接同步数据到 `ES` 中，会报错， `0000-00-00 00:00:00` 在 `mysql` 中是作为一个特殊值存在的，但 `java.sql.Date` 将其视为不合法的值，格式不正确，因此产生异常。
...
6. #### etl 多个入参
   ...
   > [https://github.com/alibaba/canal/issues/3149](https://github.com/alibaba/canal/issues/3149)
